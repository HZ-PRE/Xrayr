...
func compareUserList(old, new *[]api.UserInfo) (deleted, added []api.UserInfo) {
	...
	// 预估容量，减少扩容
	estimatedDiff := max(len(oldList), len(newList)) / 20 // 假设5%差异
	if estimatedDiff < 100 {
		estimatedDiff = 100
	}

	deleted = make([]api.UserInfo, 0, estimatedDiff)
	added = make([]api.UserInfo, 0, estimatedDiff)

	// 使用更紧凑的map标记
	// 假设api.UserInfo不大，直接作为key
	oldMap := make(map[api.UserInfo]struct{}, len(oldList))

	// 批量填充旧map
	for i := 0; i < len(oldList); i++ {
		oldMap[oldList[i]] = struct{}{}
	}
